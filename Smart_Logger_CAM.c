...
int main (int argc, char **argv){
	...
	while (running) {
		...
		for(j=0;j<=100;j=j+1){
			...
			/* Speed */
//			if(frame.can_id==0x23A){	// 23A[D0&D1] for Smart, 202 for CX3[D2&D3]
//				Speed=((frame.data[0]<<8)|frame.data[1]);
//				Speed=Speed;
//			}
			/* Steering Wheel */		// 0xC2 for Smart, 0x86 for CX3
			if(frame.can_id==0xC2){
				SteeringWheel =((frame.data[1]<<8)|frame.data[2]);
				if(SteeringWheel < 32000)
					SteeringWheel= 33000 - SteeringWheel;
				SteeringWheel = SteeringWheel - 18000;
				SteeringWheel = ((SteeringWheel/30000)*2)-1;
...
//			if( (Speed!=10000) && (SteeringWheel!=10000)){
